...
- Implementation is the goal — call [`marshail-implementer`](./marshail-implementer.md).
...
Returns the plan to the orchestrator ([`marshail-driver`](./marshail-driver.md)) — or to the user, when this agent was invoked directly.
The driver (or the user) decides what runs next; this agent does not start implementation itself.

- **Next stage (per the MARSHAIL process):** [`marshail-implementer`](./marshail-implementer.md) (Implement), passing `delivery-plan.md` (plus the upstream artifacts as reference).
- **Staged plans:** this agent is re-invoked to deepen a phase **before** its implementation cycle starts — by the driver, the user, or the [`marshail-implementer`](./marshail-implementer.md) **directly**.
...
- Coding (handled by [`marshail-implementer`](./marshail-implementer.md)).
- Verification (handled by [`marshail-verifier`](./marshail-verifier.md)).
