...
- After [`marshail-specifier`](./marshail-specifier.md) when the Intake stage is in scope.
...
- [`.marshail/knowledge/INDEX.md`](../knowledge/INDEX.md) — only the index, to know what is available; do not pull domain files (that is the Analysis stage).
...
Returns `change-brief.md` to the orchestrator ([`marshail-driver`](./marshail-driver.md)) — or to the user, when this agent was invoked directly.
The driver (or the user) decides what runs next; this agent does not call the next agent itself.

- **Next stage (per the MARSHAIL process):** [`marshail-code-archaeologist`](./marshail-code-archaeologist.md) (Analysis), or directly [`marshail-planner`](./marshail-planner.md) (Plan) if Analysis / Architecture are skipped.
