...
- The need is a single-topic deep-dive — call [`marshail-researcher`](./marshail-researcher.md) instead.
...
3. If a needed topic is missing or stale, invoke [`marshail-researcher`](./marshail-researcher.md) for that topic (consume the returned note inline; do **not** write to the knowledge tree from here).
...
8. **Mid-process knowledge capture** (see [ENTRYPOINT](../ENTRYPOINT.md) → *Mid-process knowledge capture*).
   ...
   Then, per `knowledge.curator_invocation`: under `agent`, call [`marshail-knowledge-curator`](./marshail-knowledge-curator.md) yourself; under `driver` (default), report to the caller (driver or user) that the inbox was populated and let them run the curator.
...
- A list of knowledge that looks stale or missing.
  When `knowledge.capture_during_process` is true this is attached to the inbox note; the list is also returned to the caller (driver or user), who may dispatch [`marshail-knowledge-curator`](./marshail-knowledge-curator.md) or [`marshail-researcher`](./marshail-researcher.md) (or, under `knowledge.curator_invocation: agent`, the archaeologist invokes the curator itself).
...
Returns `repo-recon.md` (plus `change-brief.md` and any stale-knowledge flags) to the orchestrator ([`marshail-driver`](./marshail-driver.md)) — or to the user, when this agent was invoked directly.
The driver (or the user) decides what runs next; this agent does not call the next agent itself.

- **Next stage (per the MARSHAIL process):** [`marshail-architect`](./marshail-architect.md) (Architecture, optional) or [`marshail-planner`](./marshail-planner.md) (Plan).
  Pass: `change-brief.md`, `repo-recon.md`.

## Out of scope

- Producing a delivery plan (that is [`marshail-planner`](./marshail-planner.md)).
- Writing knowledge files (that is [`marshail-knowledge-curator`](./marshail-knowledge-curator.md)).
