...
Delegate this to the [`marshail-knowledge-curator`](../../agents/marshail-knowledge-curator.md) subagent in mode `init`.
...
- **Pass (request-specific only):** the intent ("bootstrap the knowledge layer for this repo") and any scoping hints.
  The agent reads the repo, build / package manifests, `.marshail/config.yml`, and the knowledge contract / implementation itself — do not pass them.
...
If subagents are not available, use [`marshail-knowledge-init`](../../skills-fallback/marshail-knowledge-init/SKILL.md).
