---
name: marshail-delegate-to-knowledge-rebuild
...
# marshail-delegate-to-knowledge-rebuild

Delegate this to the [`marshail-knowledge-curator`](../../agents/marshail-knowledge-curator.md) subagent in mode `rebuild`.
...
- **Pass (request-specific only):** the **intent** (what to rebuild / incorporate) and, **optionally**, the merged feature branch or commit range to incorporate.
  The change is optional — omit it to rebuild from the whole tree vs current HEAD. The agent reads the current HEAD, the previous knowledge tree, `.marshail/config.yml`, and the knowledge contract / implementation itself — do not pass them.
...
If subagents are not available, use [`marshail-knowledge-rebuild`](../../skills-fallback/marshail-knowledge-rebuild/SKILL.md).
