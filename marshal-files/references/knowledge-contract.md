...
- how topic files or records expose a stable identity, summary, scope, and
  freshness metadata;
- how agents select the smallest useful knowledge slice for a task;
- how updates are proposed, reviewed, and applied under
  `.marshal/config.yml` autonomy settings;
...
- how deltas from research and phase learnings are promoted into canonical
  knowledge.

## Required metadata semantics

The representation may use files, records, generated indexes, or a retrieval
sidecar, but it must expose these semantics to agents:
...
- freshness marker, usually date plus verified commit or equivalent source
  revision;
...
- `review`: produce a diff or equivalent reviewable change set and wait for
  human approval.
...
- changed repo paths after an implementation cycle
  (`marshal-knowledge-maintain from-changes`);
...
To replace the knowledge implementation, add a new implementation reference
under `.marshal/references/` (or another path reachable from `.marshal/`) and
set `knowledge.representation_ref` to that file. The implementation reference
must satisfy this contract and describe its storage, metadata, discovery,
staleness, update, promotion, split, merge, and rebuild rules.

Knowledge skills should change only when the workflow changes. Pure storage,
metadata, indexing, or retrieval changes belong in the implementation
reference.