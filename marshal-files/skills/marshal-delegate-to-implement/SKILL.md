...
Delegate this to the [`marshal-implementer`](../../agents/marshal-implementer.md) subagent. Do **not** inline the workflow — the agent owns it.
...
- **On result:** when the PR boundary is filled, hand off to [`marshal-delegate-to-verify`](../marshal-delegate-to-verify/SKILL.md). After the cycle, if tracked code paths changed, hand off to [`marshal-delegate-to-knowledge-maintain`](../marshal-delegate-to-knowledge-maintain/SKILL.md) (mode `from-changes`). If assumptions break, hand back to [`marshal-delegate-to-plan`](../marshal-delegate-to-plan/SKILL.md) for the affected phase.

## Fallback (no-subagent environments)

If subagents are not available, use [`marshal-implement`](../../skills-fallback/marshal-implement/SKILL.md). Source of truth: [`marshal-implementer.md`](../../agents/marshal-implementer.md).
