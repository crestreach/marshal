...
description: MARSHAL stage 7 (Learn / improve). Delegate when the user asks to "merge the phase learnings", "produce learning-rollup.md", "promote learnings into AGENTS.md / README / rules / skills / subagents / prompts / checklists", "draft a new rule from the learnings", "draft a new skill / subagent from the learnings", "promote durable items into the knowledge layer", "close the loop on this change", "what should we keep from this change?". Also delegate after stage 6 (or 5c if rollout was skipped) for any change with phase learning files worth promoting. The subagent merges and dedupes learnings, groups them into buckets, proposes per-bucket diffs (human-approved), and routes the knowledge bucket via marshal-knowledge-curator mode from-learning.
---
