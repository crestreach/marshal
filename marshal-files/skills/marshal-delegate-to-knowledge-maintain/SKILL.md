...
Delegate this to the [`marshal-knowledge-curator`](../../agents/marshal-knowledge-curator.md) subagent in mode `from-changes`, `from-learning`, or `rescan`.
...
- **On result:** under `knowledge.autonomy: auto` (default) it is already applied — read the summary.
  ...
  For larger restructures, escalate to [`marshal-delegate-to-knowledge-rebuild`](../marshal-delegate-to-knowledge-rebuild/SKILL.md).

## Fallback (no-subagent environments)

If subagents are not available, use [`marshal-knowledge-maintain`](../../skills-fallback/marshal-knowledge-maintain/SKILL.md).
