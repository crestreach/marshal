...
Returns the rollout note to the orchestrator ([`marshal-driver`](./marshal-driver.md)) — or to the user, when this agent was invoked directly.
The driver (or the user) decides what runs next; this agent does not call the next agent itself.

- **Next stage (per the MARSHAL process):** typically [`marshal-learner`](./marshal-learner.md) (Learn), passing `rollout-note.md` plus pointers to all phase learning files.
