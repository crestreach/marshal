...
- **`init`** — first-time setup of `.marshal/knowledge/` in a new repo (typically dispatched by `marshal-init`).
...
- [`.marshal/config.yml`](../config.yml) — autonomy, size policy, and the contract / implementation refs.
...
Returns to its caller — the orchestrator ([`marshal-driver`](./marshal-driver.md)), another agent, or the user when invoked directly.
...
- Authoring net-new repo knowledge from scratch on a topic the curator cannot derive — call [`marshal-researcher`](./marshal-researcher.md) first.
