...
MARSHAL Analysis stage — see [marshal.md](../../marshal.md). Optional;
...
[activation-protocol](../references/activation-protocol.md)) — it loads
the knowledge entry point to orient, then descends only into the topics
the brief touches.
...
2. Read the entry point + indexes; pull only the knowledge whose
   coverage plausibly intersects the brief.
3. If a needed topic is missing or stale, invoke
   [`marshal-researcher`](./marshal-researcher.md) for that topic
   (consume the returned note inline; do **not** write to the
...
- `repo-recon.md` per the Analysis template (likely bounded context;
