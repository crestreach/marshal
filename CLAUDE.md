...
| [`marshal-files/`](./marshal-files) | The MARSHAL source tree for *this* product repo. In a consumer repo this same tree is installed as `.marshal/`. Holds entrypoint, config, knowledge, references, plus `marshal-files/{skills,skills-fallback,agents,rules}/` (the canonical built-in MARSHAL skills, subagents, and rules — all named `marshal-*`) and `marshal-files/extensions/{skills,agents,rules}/` (repo-specific extensions, all named `mx-*` at creation). |
