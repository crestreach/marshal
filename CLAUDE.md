...
- **Built-in MARSHAL assets** (lifecycle skills, MARSHAL subagents, rules) use the `marshal-` prefix. They are authored under `marshal-files/{skills,skills-fallback,agents,rules}/`. `marshal-promote-assets` copies them into `.agent-config/` **as-is**, keeping the `marshal-` prefix (no reprefixing).
- **Repo-specific MARSHAL extensions** generated by `marshal-learner` (or hand-authored) live under `marshal-files/extensions/{skills,agents,rules}/` and use the `mx-` prefix **at creation time**. `marshal-promote-assets` copies them into `.agent-config/` as-is, keeping the `mx-` prefix. They survive MARSHAL upgrades because they live outside the built-in folders.

## Scope

Most work here is editing the process spec and adding supporting tooling (skills, prompts, templates, examples). Treat the current spec and assets as a working version. The process is still actively being shaped, so expect conventions to keep evolving.
