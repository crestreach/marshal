#!/usr/bin/env bats
# Tests for scripts/install-marshail.sh — config.yml reconcile behavior:
...
setup() {
  TEST_HOME="$(mktemp -d "${BATS_TEST_TMPDIR:-/tmp}/marshail_conf.XXXXXX")"
  TAR_SRC="$(mktemp -d "${BATS_TEST_TMPDIR:-/tmp}/marshail_conf_tar.XXXXXX")"
...
@test "config: fresh install creates config.yml from the template" {
  ...
  grep -Eq '^[[:space:]]*autonomy:[[:space:]]*auto' "$TEST_HOME/.marshail/config.yml"
  grep -Eq '^[[:space:]]*skill_flavor:[[:space:]]*delegate' "$TEST_HOME/.marshail/config.yml"
  [[ "$output" == *"creating"* ]]
  [[ "$output" == *"config.yml"* ]]
}
...
@test "config: re-install preserves an edited value" {
  ...
  awk '/^[[:space:]]*skill_flavor:/ { sub(/delegate/, "fallback") } { print }' \
    "$TEST_HOME/.marshail/config.yml" > "$TEST_HOME/.marshail/config.yml.tmp"
  mv "$TEST_HOME/.marshail/config.yml.tmp" "$TEST_HOME/.marshail/config.yml"
  cp "$TEST_HOME/.marshail/config.yml" "$TEST_HOME/conf.before"

  run_install --no-cyncia --no-sync
  [ "$status" -eq 0 ]
  diff -u "$TEST_HOME/conf.before" "$TEST_HOME/.marshail/config.yml"
  grep -Eq '^[[:space:]]*skill_flavor:[[:space:]]*fallback' "$TEST_HOME/.marshail/config.yml"
...
@test "config: adds a schema property missing from an existing file (no-TTY -> yes)" {
  ...
  grep -v 'topic_max_lines' "$TEST_HOME/.marshail/config.yml" > "$TEST_HOME/.marshail/config.yml.tmp"
  mv "$TEST_HOME/.marshail/config.yml.tmp" "$TEST_HOME/.marshail/config.yml"
  ! grep -q 'topic_max_lines' "$TEST_HOME/.marshail/config.yml"

  run_install --no-cyncia --no-sync
  [ "$status" -eq 0 ]
  grep -Eq '^[[:space:]]*topic_max_lines:[[:space:]]*400' "$TEST_HOME/.marshail/config.yml"
...
@test "config: keeps a property no longer in the schema by default (no-TTY -> no)" {
  ...
  # Append a property MARSHAIL no longer knows about, under the sync section.
  printf '  legacy_option: keepme\n' >> "$TEST_HOME/.marshail/config.yml"
  ...
  grep -q 'legacy_option' "$TEST_HOME/.marshail/config.yml"
}
