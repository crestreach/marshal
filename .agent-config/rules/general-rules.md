---
description: General authoring rules that apply to every MARSHAL change.
always-apply: true
---
...
- **Keep everything in sync.**
  ...
  That includes connected agents and skills, `marshal.md`, `ENTRYPOINT.md`, the references, the examples under `examples/`, and any knowledge stubs.
  Documentation, agents, skills, and examples must not drift out of step with each other.
- **Prefer names over numbers** when referring to MARSHAL stages in agents, skills, and docs, so renumbering a stage does not force edits across many files.
